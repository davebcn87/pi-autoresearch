...
TESTS_RUN=$((TESTS_RUN + 1))
new_config='{"type":"config","name":"test","metricName":"ms","metricUnit":"ms","bestDirection":"lower","repo":"shopify/playground"}'
repo_val=$(echo "$new_config" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('repo',''))" 2>/dev/null || echo "error")
if [[ "$repo_val" == "shopify/playground" ]]; then
  pass "New JSONL config with repo field round-trips correctly"
else
  fail_test "New JSONL round-trip" "Expected 'shopify/playground', got '$repo_val'"
fi
