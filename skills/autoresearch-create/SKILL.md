...
- **`init_experiment`** — configure session (name, metric, unit, direction, optional `reset_policy`). Call again to re-initialize with a new baseline when the optimization target changes.
...
If `reset_policy` is `on_finish`, update it before `log_experiment` whenever the next fresh session will need the new context.
...
- **Fresh-session handoff:** if `reset_policy` is `on_finish`, assume the extension will start a clean session right after `log_experiment`. Persist anything important first.
