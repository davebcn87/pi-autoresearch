...
export default function autoresearchExtension(pi: ExtensionAPI) {
  ...
  /** Build a markdown summary of the autoresearch session for PR description */
  function generatePRSummary(selectedExperiments: ExperimentResult[]): string {
    const cur = currentResults(state.results, state.currentSegment);
    const totalCount = cur.length;
    const discarded = cur.filter((r) => r.status === "discard").length;
    const crashed = cur.filter((r) => r.status === "crash").length;
    const checksFailed = cur.filter((r) => r.status === "checks_failed").length;
    const baseline = state.bestMetric;

    // Find best primary metric among selected experiments
    let bestPrimary: number | null = null;
    for (const r of selectedExperiments) {
      if (r.metric > 0) {
    ...
    let summary = `## 🔬 Autoresearch Summary\n\n`;
    if (state.name) summary += `**Session:** ${state.name}\n\n`;

    summary += `### Results\n\n`;
    summary += `- **Total experiments:** ${totalCount}\n`;
    summary += `- **Included in PR:** ${selectedExperiments.length}\n`;
    if (discarded > 0) summary += `- **Discarded:** ${discarded}\n`;
    if (crashed > 0) summary += `- **Crashed:** ${crashed}\n`;
    if (checksFailed > 0) summary += `- **Checks failed:** ${checksFailed}\n`;
    summary += `\n`;

    if (baseline !== null) {
      summary += `### ${state.metricName}\n\n`;
      summary += `- **Baseline:** ${formatNum(baseline, state.metricUnit)}\n`;
      if (bestPrimary !== null && bestPrimary !== baseline && baseline !== 0) {
        const pct = ((bestPrimary - baseline) / baseline) * 100;
        const sign = pct > 0 ? "+" : "";
        summary += `- **Best:** ${formatNum(bestPrimary, state.metricUnit)} (${sign}${pct.toFixed(1)}%)\n`;
      }
      summary += `\n`;
    }

    if (selectedExperiments.length > 0) {
      summary += `### Included Experiments\n\n`;
      for (const r of selectedExperiments) {
        summary += `- \`${r.commit}\` ${r.description} — ${formatNum(r.metric, state.metricUnit)}\n`;
      }
      summary += `\n`;
    }

    summary += `_Generated by [pi-autoresearch](https://github.com/shopify-playground/pi-autoresearch)_\n`;
    return summary;
  }

  /**
   * Show a multi-select overlay for the user to pick which kept experiments to include.
   * Returns the selected experiments, or null if cancelled.
   */
  async function selectExperiments(ctx: ExtensionContext, kept: ExperimentResult[]): Promise<ExperimentResult[] | null> {
    if (kept.length <= 1) return kept; // Skip selection if only 0–1 experiments

    return ctx.ui.custom<ExperimentResult[] | null>(
      (tui, theme, _kb, done) => {
        // All selected by default
        const selected = new Set<number>(kept.map((_, i) => i));
        let cursor = 0;
        let scrollOffset = 0;
        ...
        return {
          render(width: number): string[] {
            ...
            const viewportRows = Math.max(4, termH - 6);
            const out: string[] = [];
            ...
            out.push(
              truncateToWidth(
                ...
                theme.fg("accent", " 🔬 Select experiments for PR ") +
                theme.fg("borderMuted", "─".repeat(Math.max(0, width - 34))),
                width
              )
            );
            out.push(
              truncateToWidth(
                `  ${theme.fg("muted", `${selectedCount}/${kept.length} selected`)}` +
                theme.fg("dim", "  │  space: toggle  a: all  n: none  enter: confirm  esc: cancel"),
                width
              )
            );
            out.push(
              truncateToWidth(
                `  ${theme.fg("borderMuted", "─".repeat(width - 4))}`,
                width
              )
            );
            ...
            // Experiment rows
            const endIdx = Math.min(kept.length, scrollOffset + viewportRows);
            for (let i = scrollOffset; i < endIdx; i++) {
              ...
              // Find the global run number for this experiment
              const globalIdx = state.results.indexOf(r);
              const runNum = globalIdx >= 0 ? `#${globalIdx + 1}` : "";

              const metricStr = formatNum(r.metric, state.metricUnit);
              const baseline = state.bestMetric;
              let deltaStr = "";
              if (baseline !== null && baseline !== 0 && r.metric !== baseline) {
                const pct = ((r.metric - baseline) / baseline) * 100;
                const sign = pct > 0 ? "+" : "";
                const color = isBetter(r.metric, baseline, state.bestDirection) ? "success" : "error";
                deltaStr = theme.fg(color, ` ${sign}${pct.toFixed(1)}%`);
              }

              const descW = Math.max(10, width - 40);
              const desc = r.description.length > descW
                ? r.description.slice(0, descW - 1) + "…"
                : r.description;

              const line =
                `  ${pointer}${checkbox} ` +
                theme.fg("accent", r.commit.padEnd(8)) +
                theme.fg(isCursor ? "text" : "muted", metricStr.padEnd(12)) +
                deltaStr.padEnd(10) +
                theme.fg("dim", `${runNum} `.padEnd(5)) +
                theme.fg(isCursor ? "text" : "muted", desc);

              out.push(truncateToWidth(line, width));
            }

            // Scroll indicator
            if (kept.length > viewportRows) {
              const above = scrollOffset;
              const below = kept.length - endIdx;
              const parts: string[] = [];
          ...
          handleInput(data: string): void {
            ...
            if (matchesKey(data, "return")) {
              const result = kept.filter((_, i) => selected.has(i));
              done(result.length > 0 ? result : null);
              return;
            }
            // Navigation
            if (matchesKey(data, "up") || data === "k") {
              ...
              cursor = Math.min(kept.length - 1, cursor + 1);
            }
            // Toggle
            else if (data === " ") {
            ...
            // Select all
              ...
              for (let i = 0; i < kept.length; i++) selected.add(i);
            }
            // Select none
            else if (data === "n") {
              selected.clear();
  ...
  /**
   * Prompt the user to create a PR at the conclusion of an autoresearch session.
   * Shows a multi-select for experiments, then sends a message to the agent to create the PR.
   */
  async function promptCreatePR(ctx: ExtensionContext) {
    if (!ctx.hasUI) return;

    const cur = currentResults(state.results, state.currentSegment);
    const kept = cur.filter((r) => r.status === "keep");

    // Don't prompt if there are no kept experiments (nothing to PR)
    if (kept.length === 0) {
      ctx.ui.notify("No kept experiments — nothing to create a PR for.", "info");
      return;
    }

    // Let the user select which experiments to include
    const selectedExperiments = await selectExperiments(ctx, kept);
    if (!selectedExperiments || selectedExperiments.length === 0) {
      ctx.ui.notify("PR creation cancelled.", "info");
      return;
    }

    // Find best primary metric among selected
    let bestPrimary: number | null = null;
    for (const r of selectedExperiments) {
      if (r.metric > 0) {
        if (bestPrimary === null || isBetter(r.metric, bestPrimary, state.bestDirection)) {
    ...
    const ok = await ctx.ui.confirm(
      "🔬 Create Pull Request?",
      `${selectedExperiments.length} experiment${selectedExperiments.length === 1 ? "" : "s"} selected, ★ ${state.metricName}: ${formatNum(bestPrimary, state.metricUnit)}${deltaStr}.\n\nCreate a PR with these changes?`
    );

    if (!ok) {
      ctx.ui.notify("PR creation skipped.", "info");
      return;
    }

    const summary = generatePRSummary(selectedExperiments);

    // Build a list of commits to cherry-pick if user deselected some
    const allKeptCommits = kept.map((r) => r.commit);
    const selectedCommits = selectedExperiments.map((r) => r.commit);
    const deselectedCommits = allKeptCommits.filter((c) => !selectedCommits.includes(c));

    let revertInstructions = "";
    if (deselectedCommits.length > 0) {
      revertInstructions =
        `\n\nThe user deselected ${deselectedCommits.length} experiment(s). ` +
        `Revert these commits before creating the PR: ${deselectedCommits.map((c) => `\`${c}\``).join(", ")}. ` +
        `Use \`git revert --no-edit <commit>\` for each one.`;
    }

    // Send a message to the agent to create the PR
    pi.sendUserMessage(
      `Create a pull request for the autoresearch results on the current branch. ` +
      `Use \`gh pr create\` (GitHub CLI). ` +
      `Use this as the PR body:\n\n${summary}\n\n` +
      `For the PR title, use something like: "${state.name || "Autoresearch optimization"}". ` +
      `Make sure you're not on the main/master branch before creating the PR. ` +
  ...
  const autoresearchHelp = () =>
    [
      "Usage: /autoresearch [off|clear|pr|<text>]",
      "",
      "<text> enters autoresearch mode and starts or resumes the loop.",
      "off   leaves autoresearch mode (prompts to create a PR if experiments were kept).",
      "pr    prompts to create a pull request with the current session's results.",
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (limitReached) {
        ...
        // Prompt user to create a PR (non-blocking, fires after tool returns)
        setTimeout(() => promptCreatePR(ctx), 500);
      }
  ...
  pi.registerCommand("autoresearch", {
    ...
    handler: async (args, ctx) => {
      ...
      if (command === "off") {
        ...
        // Prompt to create a PR if there are kept experiments
        const cur = currentResults(state.results, state.currentSegment);
        const hasKept = cur.some((r) => r.status === "keep");
        if (hasKept) {
          await promptCreatePR(ctx);
        }
        return;
      }
